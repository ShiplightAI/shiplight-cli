/**
 * right_click action - Right-click an element
 */
...
export class RightClickAction implements IAction {
  async execute(page: Page, actionEntity: ActionEntity, agentServices: AgentServices): Promise<void> {
    ...
    if (locator) {
      ...
      throw new Error('No locator found for right_click action');
    }
  }
  ...
  transpile(actionEntity: ActionEntity): string[] {
    ...
    if (parts.length === 0) {
      return [`await agent.execAction("right_click", page, {});`];
    }

    return [
      `await agent.execAction("right_click", page, {`,
      ...parts.map(p => `  ${p},`),
      `});`
    ];
  }
...
export const RightClickToolSchema = z.object({
  element_index: z.number().int().describe('Index of the element to right-click'),
...
export function registerRightClickTool(registry: ToolRegistry, action: IAction) {
  registry.register({
    name: 'right_click',
    description: 'Right-click an interactive element.',
    ...
    async execute(args, ctx) {
      ...
      try {
        ...
        if (!domElement) {
          return {
            ...
            actionEntity: createErrorActionEntity(
              `Right-click element ${element_index}`,
              'right_click',
              { index: element_index }
            ),
          };
        ...
        const description = actionDescription || `Right-click element ${element_index}`;
        ...
        return {
          success: true,
          actionEntity,
          message: `Right-clicked element ${element_index}`,
        };
      } catch (error) {
        return {
          success: false,
