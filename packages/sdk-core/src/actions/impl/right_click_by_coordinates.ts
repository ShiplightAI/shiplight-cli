/**
 * right_click_by_coordinates action - Right-click at specific coordinates
 */
...
export class RightClickByCoordinatesAction implements IAction {
    ...
    if (typeof actionData.kwargs.relative_x === 'number' && typeof actionData.kwargs.relative_y === 'number') {
      ...
      if (locator) {
        ...
        if (box) {
          ...
          await page.mouse.click(x, y, { button: 'right', delay: 100 });
          await page.waitForTimeout(200);
        }
      } else {
      ...
      await page.mouse.move(actionData.kwargs.x, actionData.kwargs.y);
      await page.waitForTimeout(200);
      await page.mouse.click(actionData.kwargs.x, actionData.kwargs.y, { button: 'right' });
      await page.waitForTimeout(200);
    } else {
  ...
  transpile(actionEntity: ActionEntity): string[] {
    ...
    return [
      `await agent.execAction("right_click_by_coordinates", page, {`,
      ...parts.map(p => `  ${p},`),
      `});`
    ];
  }
