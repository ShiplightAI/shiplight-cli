/**
 * click_by_coordinates action - Click at specific coordinates
 */
...
export class ClickByCoordinatesAction implements IAction {
  async execute(page: Page, actionEntity: ActionEntity, agentServices: AgentServices): Promise<void> {
    ...
    if (typeof actionData.kwargs.relative_x === 'number' && typeof actionData.kwargs.relative_y === 'number') {
      ...
      await page.waitForTimeout(200);
      await page.mouse.click(actionData.kwargs.x, actionData.kwargs.y);
      await page.waitForTimeout(500);
    } else {
  ...
  transpile(actionEntity: ActionEntity): string[] {
    ...
    return [
      `await agent.execAction("click_by_coordinates", page, {`,
      ...parts.map(p => `  ${p},`),
      `});`
    ];
  }
