...
| Artifact | Contents |
|---|---|
| `report-data.json` | Per-run results plus the LLM usage summary — token buckets keyed by operation, provider, bare model id and routing (FR-012). Counts only, never prompt or response content. |
...
| Cloud upload | Routed by token type: `shp_*` → the Shiplight proxy (FR-010). Any other shape resolves to no host, so the upload is skipped with a message. |
